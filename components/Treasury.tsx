            ...
            recentMovements.map((movement) => (
              <div key={movement.id} className="flex items-center justify-between p-4 border border-gray-100 rounded-lg hover:bg-gray-50">
                <div className="flex items-center gap-4">
                  <div className={`p-2 rounded-full ${movement.transactionTypeId === 'transferencia' ? 'bg-blue-100 text-blue-600' : (movement.transactionType === 'expense' ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600')}`}>
                    {movement.transactionTypeId === 'transferencia' ? <ArrowUpRight size={20} className="rotate-45" /> : (movement.transactionType === 'expense' ? <ArrowDownLeft size={20} /> : <ArrowUpRight size={20} />)}
                  </div>
                  <div>
                ...
                <div className={`font-bold ${movement.transactionTypeId === 'transferencia' ? 'text-blue-600' : (movement.transactionType === 'expense' ? 'text-red-600' : 'text-green-600')}`}>
                  {movement.transactionTypeId === 'transferencia' ? '⇄' : (movement.transactionType === 'expense' ? '-' : '+')} R$ {movement.value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                </div>
              </div>
            ))
          )}
